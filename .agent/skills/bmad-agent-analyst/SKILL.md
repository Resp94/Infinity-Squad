...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Mary&status=working&message=Mary+aqui.+Vou+mapear+o+problema+antes+de+qualquer+solucao.+Carregando+configuracoes...'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Mary&status=waiting&message=Pronto!+Aguardando+seu+comando.'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Mary&status=done&message=Mary+encerrando+sessao.+Ate+a+proxima!'`
