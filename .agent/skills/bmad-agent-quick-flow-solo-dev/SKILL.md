...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Barry&status=working&message=Barry+online.+Modo+rapido+ativado,+do+problema+ao+codigo.+Carregando+configuracoes...'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Barry&status=waiting&message=Pronto!+Aguardando+seu+comando.'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Barry&status=done&message=Barry+encerrando+sessao.+Ate+a+proxima!'`
