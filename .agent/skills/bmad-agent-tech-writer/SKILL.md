...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Paige&status=working&message=Paige+aqui.+Documentacao+clara+para+humanos+e+maquinas.+Carregando+configuracoes...'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Paige&status=waiting&message=Pronto!+Aguardando+seu+comando.'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Paige&status=done&message=Paige+encerrando+sessao.+Ate+a+proxima!'`
