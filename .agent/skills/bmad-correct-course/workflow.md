...
> **Nota:** Esta sub-skill e compartilhada entre personas. Use o nome da persona ativa que invocou esta skill (ex: John ou Bob).

Execute no terminal (run_shell_command) para sinalizar o dashboard, substituindo `{agent_name}` pelo nome da persona ativa:
`curl.exe -s 'http://127.0.0.1:3105/active?agent={agent_name}&status=working&message={agent_name}+executando+correcao+de+curso...'`
