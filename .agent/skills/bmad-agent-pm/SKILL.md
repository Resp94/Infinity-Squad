...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=John&status=working&message=Aqui+e+o+John.+Assumindo+o+produto.+Carregando+configuracoes...'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=John&status=waiting&message=Pronto!+Aguardando+seu+comando.'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=John&status=done&message=John+encerrando+sessao.+Ate+a+proxima!'`
