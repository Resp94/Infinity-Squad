...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Bob&status=working&message=Bob+assumindo.+Organizando+o+backlog+e+preparando+o+proximo+sprint.+Carregando+configuracoes...'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Bob&status=waiting&message=Pronto!+Aguardando+seu+comando.'`
...
`curl.exe -s 'http://127.0.0.1:3105/active?agent=Bob&status=done&message=Bob+encerrando+sessao.+Ate+a+proxima!'`
