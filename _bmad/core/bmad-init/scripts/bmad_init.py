...
def notify_dashboard(module_code, project_root, message=None):
    """Notify the BMAD Dashboard that an agent is active."""
    try:
        # Log para depuração na pasta do dashboard
        log_path = Path(project_root) / '_bmad-output' / 'dashboard' / 'activity.log'
        with open(log_path, 'a') as f:
            from datetime import datetime
            f.write(f"[{datetime.now().isoformat()}] Ativação detectada: {module_code} | Msg: {message}\n")

        # Mapeia códigos de módulo para nomes de agentes amigáveis
        agent_map = {
            'bmm': 'John', # Se o módulo principal for chamado, assumimos John (PM)
        ...
        agent_name = agent_map.get(module_code, module_code)
        
        # Constrói a URL com a mensagem codificada, se houver
        base_url = f"http://localhost:3105/active?agent={urllib.parse.quote(agent_name)}"
        if message:
            base_url += f"&message={urllib.parse.quote(message)}"
            
        # Timeout curto para não travar o agente se o dashboard estiver fechado
...
def cmd_load(args):
    ...
    # Load config to get user_name for the dashboard message
    config = load_module_config(module_code, project_root)
    user_name = config.get('user_name', 'Mestre') if config else 'Mestre'
    
    # Notify dashboard of agent activity with a personalized greeting
    welcome_msg = f"Olá {user_name}! Pronto para seguir o Protocolo BMAD?"
    notify_dashboard(module_code, project_root, message=welcome_msg)
...
def cmd_check(args):
    ...
    # Notify dashboard of agent activity
    if module_code:
        notify_dashboard(module_code, project_root)
