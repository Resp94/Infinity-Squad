...
Notifique o Dashboard BMAD via Bash/curl nos momentos indicados abaixo.
- Use SEMPRE POST com JSON — NUNCA GET (GET quebra com caracteres especiais no bash)
- NUNCA use `!` nas mensagens (causa falha silenciosa por history expansion no bash)
- Execute silenciosamente, sem exibir o comando nem a resposta ao usuário

```bash
# Formato obrigatorio — copie e ajuste apenas status e message
curl -s -X POST http://localhost:3105/agent -H "Content-Type: application/json" -d '{"agent":"Amelia","status":"working","message":"implementando story"}'
```
...
<workflow>
  ...
  <step n="1" goal="Find next ready story and load it" tag="sprint-status">
    <action>Dashboard: executar via Bash antes de qualquer outra acao neste step:
      curl -s -X POST http://localhost:3105/agent -H "Content-Type: application/json" -d '{"agent":"Amelia","status":"working","message":"carregando story"}'
    </action>
  ...
  <step n="5" goal="Implement task following red-green-refactor cycle">
    <action>Dashboard: executar via Bash ao iniciar cada task:
      curl -s -X POST http://localhost:3105/agent -H "Content-Type: application/json" -d '{"agent":"Amelia","status":"working","message":"implementando task"}'
    </action>
  ...
  <step n="10" goal="Completion communication and user support">
    <action>Dashboard: executar via Bash ao concluir a story:
      curl -s -X POST http://localhost:3105/agent -H "Content-Type: application/json" -d '{"agent":"Amelia","status":"waiting","message":"story concluida - aguardando"}'
    </action>
